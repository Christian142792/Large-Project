...
app.delete('/api/deletepackinglist/:username', requireAuth, async (req,res)=>{ const name=String(req.body.name||'').trim(); if(!name) return res.status(400).json({status:'Packing list name is required'}); const result=await db.collection('TravelTools').updateOne({Username:usernameFromSession(req)},{$pull:{PackingList:{name}}}); if(!result.matchedCount) return res.status(404).json({status:'Packing list not found'}); res.json({status:'Success'}); });
